    ...
    async fn watch_service(self, name: String, input: Arc<RwLock<ZInitService>>) {
        ...
        loop {
            ...
            let child = match child {
                Ok(child) => {
                    // Re-acquire the lock to update service state
                    let mut service = input.write().await;
                    service.force_set_state(State::Spawned);
                    service.set_pid(child.pid);
                    // Update stats after setting PID
                    let _ = service.update_stats();
            ...
            if !config.one_shot {
                ...
                // Spawn a task to periodically update process statistics
                let _stats_lifecycle = self.clone();
                let stats_name = name.clone();
                let stats_input = Arc::clone(&input);
                stats_handler = Some(tokio::spawn(async move {
                    let update_interval = std::time::Duration::from_secs(2);
                    loop {
                        time::sleep(update_interval).await;

                        let mut service = stats_input.write().await;
                        if !service.is_running() {
                            break;
                        }

                        if let Err(err) = service.update_stats() {
                            debug!("Failed to update stats for {}: {}", stats_name, err);
                        }
                        drop(service);
                    }
                }));
