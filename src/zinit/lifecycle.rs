...
impl LifecycleManager {
    ...
    pub async fn status<S: AsRef<str>>(&self, name: S) -> Result<crate::zinit::service::ZInitStatus> {
    ...
    pub async fn start<S: AsRef<str>>(&self, name: S) -> Result<()> {
        ...
        self.set_service_state(name.as_ref(), None, Some(Target::Up)).await;
    ...
    async fn power(&self, mode: RebootMode) -> Result<()> {
        *self.shutdown.write().await = true;
        ...
        self.kill_process_tree(dag, state_channels, shutdown_timeouts).await?;
    ...
    async fn test_loop(self, name: String, cfg: config::Service) {
        loop {
            ...
            match result {
                Ok(result) => {
                    if result {
                        self.set_service_state(&name, Some(State::Running), None).await;
                        // release
                ...
                Err(_) => {
                    self.set_service_state(&name, Some(State::TestFailure), None).await;
                }
            }
        }
    }
    ...
    async fn watch_service(self, name: String, input: Arc<RwLock<ZInitService>>) {
        ...
        loop {
            ...
            'checks: loop {
                ...
                self.set_service_state(&name, Some(State::Blocked), None).await;
            ...
            if !config.one_shot {
                let lifecycle = self.clone();
                handler = Some(tokio::spawn(lifecycle.test_loop(name.clone(), config.clone())));
            }
